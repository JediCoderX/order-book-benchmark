...
namespace {
LockFreeOrderBook makeBook() { return LockFreeOrderBook(0, 1000, 1000, 1000); }
} // namespace

TEST(lock_free_resting_order_with_no_cross_has_no_trades) {
    auto book = makeBook();
...
TEST(lock_free_crossing_order_fully_fills_resting_order) {
    auto book = makeBook();
...
TEST(lock_free_crossing_order_partially_fills_and_rests_remainder) {
    auto book = makeBook();
    book.addOrder(1, Side::Sell, 100, 10, 0);
...
TEST(lock_free_fifo_priority_within_price_level) {
    auto book = makeBook();
...
TEST(lock_free_cancel_removes_resting_order) {
    auto book = makeBook();
...
TEST(lock_free_modify_reprices_and_can_cross) {
    auto book = makeBook();
...
TEST(lock_free_best_bid_skips_to_next_occupied_level_after_drain) {
    auto book = makeBook();
    book.addOrder(1, Side::Buy, 100, 10, 0);
...
// Disjoint-lane concurrent stress, same shape as the other lock variants
TEST(lock_free_survives_concurrent_access_from_multiple_threads) {
    constexpr int kThreads = 8;
    constexpr uint64_t kOrdersPerThread = 500;
    constexpr uint32_t kQuantity = 10;

    LockFreeOrderBook book(0, 1000, kThreads * kOrdersPerThread + 10,
...
// Many "maker" threads resting sell orders at the same single price level
// then many "taker" threads concurrently crossing against that same level at once
TEST(lock_free_survives_concurrent_matching_on_shared_price_level) {
    ...
    LockFreeOrderBook book(0, 1000,
                                 (kMakerThreads + kTakerThreads) * kOrdersPerMaker + 10,
                                 (kMakerThreads + kTakerThreads) * kOrdersPerMaker + 10);

    // makers: ids [0, kMakerThreads*kOrdersPerMaker)
    ...
    // takers: ids starting after all maker ids, same total demand as supply
    ...
    for (int t = 0; t < kTakerThreads; ++t) {
        takers.emplace_back([&book, t, &total_traded]() {
            uint64_t base_id =
                static_cast<uint64_t>(kMakerThreads) * kOrdersPerMaker +
