...
uint32_t OrderBook::matchAgainst(BookMap& opposite, uint64_t taker_id, int64_t price,
...
void OrderBook::rest(Side side, int64_t price, uint64_t order_id, uint32_t quantity,
...
std::vector<Trade> OrderBook::addOrder(uint64_t order_id, Side side, int64_t price,
...
std::vector<Trade> OrderBook::modifyOrder(uint64_t order_id, int64_t new_price,
...
uint32_t OrderBook::quantityAt(Side side, int64_t price) const {
    uint32_t total = 0;
    if (side == Side::Buy) {
